...
import { FaGraduationCap } from "react-icons/fa"; // Education category icon
  ...
  return (
    <div className="min-h-screen mt-10 bg-gray-100 text-gray-900 dark:bg-gray-900 dark:text-white">
      <div className="container mx-auto px-6 py-16">
        {/* Education Icon and Title in the Same Row */}
        <div className="flex items-center justify-center space-x-4 mb-12">
          <FaGraduationCap size={50} color="#10B981" />
          <h1 className="text-4xl font-semibold">Education Posts</h1>
        </div>
        ...
        {loading ? (
          ...
          // Post Cards Layout with New Design
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {posts.length > 0 ? (
              posts.map((post) => (
                <div
                  key={post.id}
                  className="bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden transition-all duration-300 transform hover:scale-105 cursor-pointer"
                ...
                >
                  <div className="relative">
                    ...
                    {post.imageUrl && (
                      <img
                        src={post.imageUrl}
                        alt={post.title || "Education Post"}
                        className="w-full h-56 object-cover rounded-t-lg mb-4"
                      />
                    )}
                    <div className="absolute top-4 left-4 bg-gray-800 text-white text-sm px-2 py-1 rounded-lg">
              ...
              <p className="text-center text-gray-500 dark:text-gray-400">No education posts available.</p>
            )}
          </div>
        )}
      </div>
